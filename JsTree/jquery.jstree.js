        ...
        $.jstree.plugin("themes", {
            ...
            _fn: {
                set_theme: function (theme_name, theme_url) {
                    ...
                    if (!theme_url) { theme_url = '../css/style.css'; }
        ...
        $.jstree.plugin("cookies", {
            __init: function () {
                ...
                var s = this._get_settings().cookies,
					tmp;
                var pfx = s.cookie_options.prefix;

                if (!!s.save_loaded + pfx) {
                    tmp = $.cookie(s.save_loaded + pfx);
                    if (tmp && tmp.length) { this.data.core.to_load = tmp.split(","); }
                }
                if (!!s.save_opened + pfx) {
                    tmp = $.cookie(s.save_opened + pfx);
                    if (tmp && tmp.length) { this.data.core.to_open = tmp.split(","); }
                }
                if (!!s.save_selected + pfx) {
                    tmp = $.cookie(s.save_selected + pfx);
            ...
            _fn: {
                save_cookie: function (c) {
                    ...
                    if (!c) { // if called manually and not by event
                        if (s.save_loaded + pfx) {
                            this.save_loaded();
                            $.cookie(s.save_loaded + pfx, this.data.core.to_load.join(","), s.cookie_options);
                        }
                        if (s.save_opened + pfx) {
                            this.save_opened();
                            $.cookie(s.save_opened + pfx, this.data.core.to_open.join(","), s.cookie_options);
                        }
                        if (s.save_selected + pfx && this.data.ui) {
                            this.save_selected();
                            $.cookie(s.save_selected + pfx, this.data.ui.to_select.join(","), s.cookie_options);
                        }
                        return;
                    }
                    switch (c) {
    ...
    (function ($) {
        $.jstree.plugin("localStorage", {
            __init: function () {
                ...
                var tmp;
                var pfx = s.localStorage_options.prefix;
                if (!!s.save_loaded + pfx) {
                    tmp = localStorage.getItem(s.save_loaded + pfx);
                    if (tmp && tmp.length) { this.data.core.to_load = tmp.split(","); }
                }
                if (!!s.save_opened + pfx) {
                    tmp = localStorage.getItem(s.save_opened + pfx);
                    if (tmp && tmp.length) { this.data.core.to_open = tmp.split(","); }
                }
                if (!!s.save_selected + pfx) {
                    tmp = localStorage.getItem(s.save_selected + pfx);
                    if (tmp && tmp.length && this.data.ui) { this.data.ui.to_select = tmp.split(","); }
                }
                this.get_container()
                    .one((this.data.ui ? "reselect" : "reopen") + ".jstree", $.proxy(function () {
            ...
            _fn: {
                save_node: function (c) {
                    ...
                    if (!c) { // if called manually and not by event
                        if (s.save_loaded + pfx) {
                            this.save_loaded();
                            localStorage.setItem(s.save_loaded + pfx, this.data.core.to_load.join(","));
                        }
                        if (s.save_opened + pfx) {
                            this.save_opened();
                            localStorage.setItem(s.save_opened + pfx, this.data.core.to_open.join(","));
                        }
                        if (s.save_selected + pfx && this.data.ui) {
                            this.save_selected();
                            localStorage.setItem(s.save_selected + pfx, this.data.ui.to_select.join(","));
                        }
                        return;
                    }
                    switch (c) {
